...
import sqlalchemy
from opentelemetry.instrumentation.sqlcommenter.sqlalchemy.executor import BeforeExecuteFactory
...
class Tests(SQLAlchemyTestCase):
    ...
    def test_both_opentelemetry_and_opencensus_warn(self):
        with mock.patch(
            "opentelemetry.instrumentation.sqlcommenter.sqlalchemy.executor.logger"
...
class FlaskTests(SQLAlchemyTestCase):
    ...
    @mock.patch('opentelemetry.instrumentation.sqlcommenter.sqlalchemy.executor.get_flask_info', return_value=flask_info)
